...
def setup_activity_log(reset=True):
    ...
    print("")
    print("=" * 80)
    print(f"[{utc_now()}] ACTIVITY LOG STARTED")
...
def websocket_candle_watchdog():
    while True:
        ...
        if ws_to_close is not None:
            print("")
            print("=" * 80)
            print(f"[{utc_now()}] STALE_CANDLE_STREAM")
            ...
            journal.log_event(
                event_type="STALE_CANDLE_STREAM",
                payload={
            ...
            try:
                ...
                print("")
                print("=" * 80)
                print(f"[{utc_now()}] STALE_CANDLE_STREAM_CLOSE_ERROR")
                print(exc)
                print("=" * 80)

                journal.log_event(
                    event_type="STALE_CANDLE_STREAM_CLOSE_ERROR",
                    payload={
                        "error": str(exc)
                    }
                )
...
def start_websocket_watchdog():
    ...
    print("")
    print("=" * 80)
    print(f"[{utc_now()}] WEBSOCKET CANDLE WATCHDOG STARTED")
...
def recover_engine_position(engine, market):
    ...
    if restored:
        ...
        if engine.last_candle is not None:
            ...
            print("")
            print("=" * 80)
            print(f"[{utc_now()}] RECOVERY PRICE CHECK SKIPPED")
            print(f"Market: {market}")
            print("Reason: engine.last_candle is None")
            print("=" * 80)
